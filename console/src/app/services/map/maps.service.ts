...
import {MapExecutionLogs, MapStructure, MapTrigger} from '@maps/models';
...
export class MapsService {
  ...
  init() {
    this.socketService.getMessageAsObservable().subscribe(data => {
      if (data.type !== 'saved-map') {
        return;
      }
      if (
        (data.msg.initiator !== this.socketService.socketID) &&
        (this.currentMapId === data.msg.mapId)
      ) { // if another user or in another tab map was saved
        this.checkSyncMap();
      } else {
  ...
  checkSyncMap() {
    ...
    this.popupService.openConfirm(
      'Map Changed',
      'This map was saved in different window. Would you like to refresh window and get the latest map?',
      OK,
      'Close',
      null
    ).subscribe(result => {
