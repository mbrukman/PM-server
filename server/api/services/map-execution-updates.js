...
function _runDbFunc() {
  ...
  funcs[lastElement.func](lastElement.data)
    .then(res => {
      dbRetries = 3;
    ...
    .catch(err => {
      ...
      dbQueue = [];
      mapExecMethods.stopExecution(
        null,
        null,
        " - MongoError",
        lastElement.data.mapResultId
      );
    });
}
...
function _updateMapResult(options) {
  return MapResult.findByIdAndUpdate(
    options.mapResultId,
    { $set: options.data },
    { new: true }
  ).then(mapResult => {
...
function _addAction(options) {
  const pathToSet =
    "agentsResults.$.processes." + options.processIndex + ".actions";
  const toUpdate = {};
...
module.exports = function(methods) {
  ...
  return {
    ...
    getAndUpdatePendingExecution(mapId) {
      ...
      return MapResult.findOneAndUpdate(
        { map: ObjectId(mapId), status: statusEnum.PENDING },
        { status: statusEnum.RUNNING, startTime: new Date() },
