...
module.exports = {
  ...
  archive: (req, res) => {
    archiveService
      .archiveMaps([req.params.id], req.body.isArchive)
      .then(map => {
  ...
  duplicateMap: (req, res) => {
    ...
    hooks
      ...
      .then(duplicatedMap => {
        ...
        return mapsService.getMapStructure(
          req.params.id,
  ...
  update: (req, res) => {
    ...
    hooks
      .hookPre("map-update", req)
      .then(() => {
        return mapsService.update(mapId, req.body);
      })
      .then(map => {
        return req.body.project
          ? projectsService.updateMap(mapId, req.body.project)
          : null;
      })
      .then(() => {
        return res.send("OK");
  ...
  createStructure: (req, res) => {
    ...
    hooks
      .hookPre("map-create-structure", req)
      .then(() => {
      ...
      .catch(error => {
        ...
        req.io.emit("notification", {
          title: "Whoops...",
          message: `Error saving map structure`,
          type: "error",
          mapId: mapId
        });
  ...
  getStructureList: (req, res) => {
    hooks
      .hookPre("map-list-structures", req)
      .then(() => {
  ...
  cancelPending: (req, res) => {
    hooks
      .hookPre("map-cancel-pending", req)
      .then(() => {
        return mapsExecutionService.cancelPending(
          req.params.id,
          req.body.runId,
          req.io
        );
  ...
  currentRuns: (req, res) => {
    ...
    hooks.hookPre("map-currentruns", req).then(() => {
      ...
      return res.json(
        Object.keys(executions).reduce((total, current) => {
  ...
  execute: (req, res) => {
    ...
    if (req.body) {
      ...
      let configTokenPayload = configToken.validateAndExtractToken(
    ...
    let config =
      payload && payload.config
        ? Object.assign(configRequest, payload.config)
        : configRequest;
    let trigger =
      payload && payload.triggerMsg
        ? triggerRequest + " " + "-" + " " + payload.triggerMsg
        : triggerRequest;
    let configuration = {
      config,
    ...
    hooks
      .hookPre("map-execute", req)
      .then(() => {
        return mapsExecutionService.execute(
          req.params.id,
  ...
  stopExecution: (req, res) => {
    return res.json(
      mapsExecutionService.stop(req.params.runId, req.io, " - manually by user")
    );
  },

  logs: (req, res) => {
    hooks
      .hookPre("map-logs-list", req)
      .then(() => {
  ...
  resultDetail: (req, res) => {
    hooks
      .hookPre("map-results-detail")
      .then(() => {
        return mapsExecutionService.detail(req.params);
      })
      .then(async execResult => {
        if (!execResult && req.params.resultId && req.params.resultId != "null")
          // wrong Id
        ...
        if (!execResult.status) {
          // the old maps do not need to be mapped
          return execResult;
        }
