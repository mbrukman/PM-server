...
module.exports = {
  ...
  add: (req, res) => {
    ...
    hooks
      ...
      .then(agentsPlugin => {
        ...
        Promise.all(
          filesPaths.map(function asyncFilesPaths(filePath) {
            return new Promise((resolve, reject) => {
              agentsService
                .installPluginOnAgent(filePath, agent)
                .then(() => {})
                .catch(e => {
  ...
  groupDetail: (req, res) => {
    hooks
      ...
      .catch(error => {
        req.io.emit("notification", {
          title: "Whoops...",
          message: `Error adding agent to group`,
          type: "error"
        });
        winston.log("error", "Error creating group", error);
        res.status(500).send(error);
  ...
  addAgentToGroup: (req, res) => {
    hooks
      ...
      .then(group => {
        req.io.emit("notification", {
          title: "Excellent",
          message: `Agent was added to group`,
          type: "success"
        });
        return res.json(group);
      })
  ...
  deleteFilterFromGroup: (req, res) => {
    hooks
      .hookPre("group-remove-agent", req)
      .then(() => {
        return agentsService.deleteFilterFromGroup(
          req.params.groupId,
          req.params.index
        );
      })
  ...
  removeAgentFromGroup: (req, res) => {
    hooks
      .hookPre("group-remove-agent", req)
      .then(() => {
        return agentsService.removeAgentFromGroup(
          req.params.id,
          req.body.agentId
        );
      })
